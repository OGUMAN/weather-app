<template>
  <div class="range">
    ...
    <div class="range__ticks">
      <div class="range__tick" v-for="(hour, index) in day.range" :key="index">
        <div
          class="range__line"
...
import { mapState, mapMutations } from "@/store/helpers";
import day from "@/data/scheduleHours.json";

export default defineComponent({
  setup() {
    const { selectedHourId } = mapState();
    ...
    const rangeOnChange = (event: Event): void => {
      setSelectedHourId(parseInt((event.target as HTMLInputElement).value));
      setTimeIsChanged(true);
    };

    return { selectedHourId, rangeOnChange, day };
  },
});
</script>
